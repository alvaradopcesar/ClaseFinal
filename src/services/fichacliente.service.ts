...
import { IFichaCliente } from './../interfaces/fichacliente.interface';
...
export class FichaClienteService {
    ...
    queryFichaClienteDate(userID: number): Observable<IFichaCliente[]> {
        this.headers = new Headers({ 'Authorization': 'Bearer ' + this.authenticationService.token });
        this.options = new RequestOptions({ headers: this.headers });

        return this._http.get(environment.urlServices + '/test/fichacliente' , this.options)
